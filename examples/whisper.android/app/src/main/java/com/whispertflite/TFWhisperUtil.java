...
public class TFWhisperUtil {
    private static final String TAG = "TFWhispher";
    ...
    public boolean loadFiltersAndVocab(boolean multilingual, InputStream vocabStream) throws IOException {

        // Read vocab file
//        byte[] bytes = Files.readAllBytes(vocabFile.toPath());
//        ByteBuffer vocabBuf = ByteBuffer.wrap(bytes);
        ByteBuffer vocabBuf = null;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) { // TODO: this forces Android 13 for this feature
            vocabBuf = ByteBuffer.wrap(vocabStream.readAllBytes());
        } else {
        ...
        if (magic == 0x5553454e) {
            ...
            Log.e(TAG, "Invalid vocab file (bad magic: " + magic + ")");
            return false;
        }
